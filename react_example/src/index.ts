    ...
    try{
      ...
      if (path.startsWith('/api/users')) {
        if (path === "/api/users/create") {
          res = await userRouter.create(request, res, env);  
        }
        if (path === "/api/users/get") {
          res = await userRouter.get(request, res, env);  
        }
        if (path === "/api/users/login") {
          res = await userRouter.login(request, res, env);  
          console.log(res);
        }
        if (path === "/api/users/get_user_id") {
          res = await userRouter.getUserId(request, res, env);  
          console.log(res);
        }
        if(res.ret) {
